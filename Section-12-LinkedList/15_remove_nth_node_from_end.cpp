...
//search in linked list iterative version
...
//search in a linked list recursive version
bool isPresent_rec(Node *head, int data)
{
    if (head == NULL)
        return false;
    if (head->data == data)
    ...
    return isPresent_rec(head->next, data);
}
...
Node *middleNode(Node *head)
{
    ...
    if (fast != NULL)
    {
        return slow->next; //even length case
    }
...
int main()
{
    ...
    cout << "Length of Linked List(rec): " << length_rec(head) << endl;
    ...
    Node *head_temp = middleNode(head);
    cout << "Middle Node : ";
    printList(head_temp);
    ...
    printList(removeKthNode(head_tmp, 3));
